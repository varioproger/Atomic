...
public:
	...
	T test;
	void Test_Call(T _test, void (First ::* _fcnPtr)(void))
	{
		(_test.*_fcnPtr)();
	}
};
...
int main()
{
	...
	cout << endl << "AddBind" << endl;
	test.AddBind(&fir);
	test.AddBind(&sec);
	test.AddBind(&thr);
	test.AddBind(&fort);
	cout << endl << "Lock_Get_Instatnce" << endl;
	test.Lock_Get_Instatnce(&thr);
	test.Lock_Get_Instatnce(&fort);
	test.Lock_Get_Instatnce(&fir);
	cout << endl << "No_Lock_Get_Instatnce" << endl;
	test.No_Lock_Get_Instatnce(&thr);
	test.No_Lock_Get_Instatnce(&fort);
	test.No_Lock_Get_Instatnce(&fir);
	return 0;
}
